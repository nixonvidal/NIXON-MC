...
vl_port=8443
tag=$(jq -r '.port' /usr/local/x-ui/bin/${i}.log)-regreso-al-origen-vless-ws-tls
else
vl_port=8880
tag=$(jq -r '.port' /usr/local/x-ui/bin/${i}.log)-volver a fuente-vless-ws
fi
...
if [[ $(jq -r '.streamSettings.security' /usr/local/x-ui/bin/${i}.log) == 'tls' ]]; then
vm_port=8443
tag=$(jq -r '.port' /usr/local/x-ui/bin/${i}.log)-volver al origen-vmess-ws-tls
else
vm_port=8880
tag=$(jq -r '.port' /usr/local/x-ui/bin/${i}.log)-volver al origen-vmess-ws
fi
...
red "Actualmente x-ui no tiene un conjunto de configuración de nodo válido" && exit
fi
done
