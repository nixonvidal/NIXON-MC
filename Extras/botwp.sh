...
git clone https://github.com/Rizky878/rzky-multidevice/ >/dev/null 2>&1
...
desinst_bot() {
rm -r rzky-multidevice &>/dev/null
...
activa() {
unset PIDGEN
PIDGEN=$(ps aux|grep -v grep|grep "rzky-multidevice")
if [[ ! $PIDGEN ]]; then
cd rzky-multidevice
npm install --arch=x64 --platform=linux sharp
screen -S rzky-multidevice npm start
else
screen -S rzky-multidevice -p 0 -X quit
fi
}
#screen -S rzky-multidevice &>/dev/null
#VER INFORMACIÓN
inf(){
echo -e "$barra"
...
menu(){
unset PID_GEN
PID_GEN=$(ps x|grep -v grep|grep "rzky-multidevice")
[[ ! $PID_GEN ]] && PID_GEN="\033[1;31m[ ✖ OFF ✖] 🔴" || PID_GEN="\033[1;32m[ LINEA ] 🟢"
[[ -e rzky-multidevice/config.json ]] && php="\033[1;32m[ON]" || php="\033[1;31m[OFF]"
